export default {
    ...
    initPlugin: function (params) {
        var initParams = {
            ...
            contentY: params.contentY || 15,
            spacingX: params.spacingX || 10
        };
        ...
        //最大刻度的小值是40
        initParams.maxScale = initParams.maxScale < 40 ? 40 : initParams.maxScale;
        ...
        var division = winWidth / 40; //每个刻度的距离 分割线
        //刻度值数组
        var scaleValueList = [];
        ...
        function drawRuler(count) {
            ...
            for (var i = 0; i < initParams.maxScale; i++) {
                ...
                if (i % 1 === 0) {
                    cxt.moveTo(division * i - count * division, 14 + initParams.contentY);
                    cxt.strokeStyle = initParams.color ? initParams.color : "#999";
                    cxt.lineTo(division * i - count * division, Math.floor(initParams.height * 0.45) + initParams.contentY);
                }
                if (i % 5 === 0) {
                    cxt.moveTo(division * i - count * division, 8 + initParams.contentY);
                    cxt.strokeStyle = initParams.color ? initParams.color : "#303133";
                    cxt.lineTo(division * i - count * division, Math.floor(initParams.height * 0.52) + initParams.contentY);
                }
                cxt.stroke();
                cxt.restore();
                cxt.closePath();
            ...
            cxt.beginPath();
            cxt.font = "12px Arial";
            cxt.fillStyle = initParams.color ? initParams.color : "#A3A7AD";
            cxt.textAlign = "center";
            cxt.textBaseline = "middle";
            scaleValueList.forEach(function (num, i) {
                cxt.fillText(num.toString() + `${i==0?'元':'万'}`, (division * i * 5) - (count * division), Math.floor(initParams.height * 0.5) - initParams.contentY);
            });
            cxt.closePath();
            ...
            cxt.lineWidth = 2;
            cxt.lineCap = "round";
            cxt.moveTo((winWidth / 2), 5 + initParams.contentY);
            cxt.lineTo((winWidth / 2), Math.floor(initParams.height * 0.54) + initParams.contentY);
            cxt.stroke();
            cxt.restore();
            cxt.closePath();
        ...
        if (window.devicePixelRatio) {
            canvas.style.transform = "scale(" + ((1 / window.devicePixelRatio) + 0.07) + ")";
