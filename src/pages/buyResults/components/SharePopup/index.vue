<template>
  <popup v-model="show" position="bottom" :show-mask="true" :hide-on-blur="true" :is-transparent="false" @on-show="$modalHelper.open" @on-hide="$modalHelper.close">
    <!-- <div class="header" v-show="!isLogin">
      <span class="text">登录后分享有机会获得现金红包</span>
      <a class="btn" href="javascript:void(0);" @click="$emit('clickLogin')">登录</a>
    </div> -->
    <div class="box">
      <div class="content">
...
.header {
  ...
  .btn {
    font-size: 25px;
    color: #fff;
    background: linear-gradient(to top, #e3a334, #f5ca4b);
    padding: 8px 50px;
    border-radius: 6px;
  }
}
