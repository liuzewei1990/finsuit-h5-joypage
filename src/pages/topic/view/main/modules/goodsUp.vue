  ...
  <div class="main-goods-up" v-show="goods.length">
    ...
    <div class="box-scroll">
      <ul class="box-list" :style="{width:`${boxWidth}px`}">
        <!-- <ul class="box-list"> -->
        <!-- item -->
        <li class="item dev" v-for="(item,index) in goods" :key="index" ref="item" @click="$checkLogin({action:getGoodsRedirectUrl,data:item},false)" v-track:ACB0I016.click="{handleClick:()=>{},item}">
        ...
        <li class="item dev" v-if="goods.length < 6">
          <div class="item-bank dev">
            <span class="item-bank-icon dev">
              <img :src="require('@topic/assets/images/$.png')" alt />
            </span>
      ...
      try {
        ...
        if (data.is_enabled == "0") {
          ...
          if (data.redirect == 2) {
            ...
            let http =
              this.$store.state.APP_FLAG == "BC" ? "besharp" : "pincai";
