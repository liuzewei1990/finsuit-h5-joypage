      ...
      <TabviewItem class="tabview-item" v-for="(itemObj,index) in teamData" :key="index" ref="TabviewItem">
        <div class="view-container view-list" id="view-container" ref="view-container">
          <div class="list-item">
            ...
            <TeamItemMin class="mar-bottom-20" :item="item.uiData" v-for="(item,index) in itemObj.LIST" :key="index" @click.native="$checkLogin({action:handleClickItem,data:item})" @reminBtn="handleClickReminBtn(item)"></TeamItemMin>
            <!-- <pre>
                {{itemObj}}
            </pre> -->
            <!-- 数据为空 -->
...
export default {
  ...
  computed: {
    ...
    teamData () {
      return this.teamDatas.map(groupItem => {
        groupItem.LIST = (groupItem.LIST || []).map(item => {
          item.uiData = this.uiData(item);
          return item;
        });
        return groupItem;
      })
  ...
  methods: {
    ...
    async handleClickItem (item) {
      ...
      let data = await this.$api.postCheckTeamStatusPC({ TEAM_ID: item.TEAM_ID });
    ...
    uiData (item) {
      ...
      let uiData = {};
      // teamId
      uiData.teamId = item.TEAM_ID;
      // 团状态
      uiData.teamStatus = item.START_STATUS;
      // 提醒
      uiData.isRemind = item.IS_REMIND;
      // 银行名称
      uiData.orgName = item.ORG_NAME;
      // 产品名称
      uiData.prdName = item.PRD_NAME;
      // 利率
      uiData.pedRate = item.PRD_RATE;
      // 加息
      uiData.couponRate = item.COUPON_RATE;
      // 利率描述
      uiData.prdRateText = item.RATE_BABEL_TEXT;
      // 标签
      uiData.tags = this.$utils.toStrType(item.LABEL_TEXT1).split(',');
      // 团进度
      uiData.teamSchedule = item.TEAM_SCHEDULE;
      // 剩余金额
      uiData.collagedAmount = item.COLLAGED_AMOUNT;
      // 总金额
      uiData.totalAmount = item.TEAM_TOTAL_AMOUNT;
      return uiData;
    }
  },
}
