        ...
        <div class="view-container view-list" id="view-container" ref="view-container">
          ...
          <div class="list-item">
            ...
            <template v-if="itemObj.STATUS==2">
              <TeamItem class="mar-bottom-20" v-for="(item,index) in itemObj.LIST" :key="index" :item="item" @click.native="handleClickItem(item)" @countdownOver="countdownOver" @reminBtn="handleClickReminBtn"></TeamItem>
            </template>
            <!-- STATUS：3 拼团中 -->
            <template v-if="itemObj.STATUS==3">
              <TeamItem class="mar-bottom-20" v-for="(item,index) in itemObj.LIST" :key="index" :item="item" @click.native="handleClickItem(item)" @countdownOver="countdownOver"></TeamItem>
            </template>
            <!-- 数据为空 -->
          ...
          <!-- 合作银行 -->
          <ComBank class="mar-top-20" :disabled="comBankDisabled" ref="ComBank"></ComBank>
...
export default {
  ...
  methods: {
    ...
    slideChangeTransitionStart () {
      // 当tabView左右滑动时禁用页面轮播动画
      //   this.$refs["ComBank"][this.currentTabviewIndex].disabledFn(true);
      //  归位页面滚动条
