<!-- 用户积分明细流水 -->
<template>
  <section class='user-score-note-page' v-infinite-scroll="loadBottom" infinite-scroll-immediate-check="false" infinite-scroll-disabled="infiniteDisabled" infinite-scroll-distance="80">
      ...
      <ul class="list">
        <li class="list-item" v-for="(item,index) in list" :key="index">
          <div class="list-item-inner">
            <div class="icon"></div>
            <div class="content">
              <p class="title">{{item.orderCp}}</p>
              <span class="sub-title">{{item.orderTime}}</span>
            </div>
            <div class="action">
              <span v-show="item.orserType == '1'">+{{item.orderScore}}</span>
              <span v-show="item.orserType == '2'">-{{item.orderScore}}</span>
              <span v-show="item.orserType == '3'">-{{item.orderScore}}</span>
            </div>
          </div>
        </li>
      </ul>
    ...
    */
    ...
    async loadTop () {
      ...
      try {
        ...
        this.status = "刷新中";
        let data = await this.$api.getUserScoreNote(this.query).then(this.$interfaceApi.getUserScoreNote);
        this.list = data.data;
