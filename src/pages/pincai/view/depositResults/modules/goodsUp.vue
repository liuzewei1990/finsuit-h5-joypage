<template>
    <div class="main-goods-up" v-show="goods.length">
        <div class='name'>精选产品</div>
        <div class="box-scroll">
            <ul class="box-list">
                <!-- <ul class="box-list">  -->
                <!-- item -->
                <li class="item dev" v-for="(item,index) in goods" :key="index" ref="item" @click="getGoodsRedirectUrl(item)" v-track:ACB0T006.click="{handleClick:()=>{},item}">
        ...
        async getGoodsList () {
            try {
                let params = {
                    ...
                    PRD_ID: this.$route.query.proId ? this.$route.query.proId.trim() : ""
                };
                let data = await this.$api.getGoodsList(params);
                this.handleData(data);
            } catch (error) {
                console.error("获取精选产品列表：", error);
            }
        },
            ...
            try {
                ...
                if (data.is_enabled == "0") {
                    ...
                    if (data.redirect == 2) {
                        ...
                        this.$Toast(
                            "data redirect type error " + data.redirect
                        );
                    }
                }
                // 下架的活动
